...
class MaskedEdgeAttention(nn.Module):
    ...
    def __init__(self, input_dim, max_seq_len, device):
        """
    ...
    def forward(self, M, lengths, edge_ind):
        ...
        if attn_type == 'attn1':
            ...
            mask = Variable(torch.ones(alpha.size()) * 1e-10).detach().to(self.device)
            mask_copy = Variable(torch.zeros(alpha.size())).detach().to(self.device)
            ...
            scores = torch.zeros(M.size(1), self.max_seq_len, self.max_seq_len, requires_grad=True).to(self.device)
...
def batch_graphify(features, qmask, lengths, window_past, window_future, edge_type_mapping, att_model, device = torch.device('cpu')):
    """
...
#        base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future,
#                 n_classes=7, listener_state=False, context_attention='simple', dropout_rec=0.5, dropout=0.5, nodal_attention=True, avec=False, no_cuda=False):
        ...
        self.D_e = opt.emotion_rep_dim
#        self.avec = avec
        self.dropout = opt.dropout
        self.n_classes = opt.output_dim
        self.graph_hidden_size = opt.graph_hidden_size
        self.max_seq_len = opt.max_seq_len
        self.avec = opt.dataset_name == 'avec'
        self.device = opt.device

        # The base model is the sequential context encoder.
        if self.base_model == 'dialogue-rnn':
            self.dialog_rnn_f = DialogueRNN(self.D_m, opt.global_state_dim, opt.party_state_dim, self.D_e, opt.listener_state, \
                                            opt.context_attention_type, opt.concat_attention_dim, opt.dropout_rec)
            self.dialog_rnn_r = DialogueRNN(self.D_m, opt.global_state_dim, opt.party_state_dim, self.D_e, opt.listener_state, \
                                            opt.context_attention_type, opt.concat_attention_dim, opt.dropout_rec)

        elif self.base_model == 'lstm':
            self.lstm = nn.LSTM(input_size=self.D_m, hidden_size=self.D_e, num_layers=2, bidirectional=True, dropout=self.dropout)

        elif self.base_model == 'gru':
            self.gru = nn.GRU(input_size=self.D_m, hidden_size=self.D_e, num_layers=2, bidirectional=True, dropout=self.dropout)


        elif self.base_model == 'none':
            self.base_linear = nn.Linear(self.D_m, 2*self.D_e)

        else:
            print ('Base model must be one of DialogRNN/LSTM/GRU')
            raise NotImplementedError 
            
        n_speakers = opt.speaker_num
        n_relations = 2 * n_speakers ** 2
        self.window_past = opt.window_past
        self.window_future = opt.window_future

        self.att_model = MaskedEdgeAttention(2*self.D_e, self.max_seq_len, self.device)
        self.nodal_attention = opt.nodal_attention

        self.graph_net = GraphNetwork(2*self.D_e, self.n_classes, n_relations, self.max_seq_len, self.graph_hidden_size, self.dropout, self.device)
    ...
    def forward(self, in_modalities):
        qmask, umask = in_modalities[-2], in_modalities[-1]
        
        U = torch.cat(in_modalities[:-2],dim=-1)
        U = U.transpose(0,1)
    ...
    #def forward(self, U, qmask, umask, seq_lengths):
        ...
        features, edge_index, edge_norm, edge_type, edge_index_lengths = batch_graphify(emotions, qmask, seq_lengths, self.window_past, self.window_future, self.edge_type_mapping, self.att_model, self.device)
        log_prob = self.graph_net(features, edge_index, edge_norm, edge_type, seq_lengths, umask, self.nodal_attention, self.avec)
        log_prob_all = torch.zeros(U.shape[1],self.max_seq_len, log_prob.shape[-1]).to(self.device)
        ind = 0
        for i,_len in enumerate(seq_lengths):
            log_prob_all[i][:_len] = log_prob[ind:ind+_len]
            ind = ind+_len
        return log_prob_all