...
#                                            zip(self.input_dims[1:],self.hidden_dims[1:],self.dropout_probs[1:])])
        ...
        self.text_factor = nn.Parameter(torch.Tensor(self.rank, self.input_dims[0] + 1, self.output_dim).to(self.device))
        self.other_factors = [nn.Parameter(torch.Tensor(self.rank, hidden_dim + 1, self.output_dim).to(self.device)) for hidden_dim in self.input_dims[1:]]
...
#            DTYPE = torch.FloatTensor
        fusion = torch.matmul(torch.cat([torch.ones(batch_size, 1).to(self.device), in_modalities[0]], dim=1),self.text_factor)
        for i in range(self.num_modalities-1):
            added_unit = torch.cat([torch.ones(batch_size, 1).to(self.device), in_modalities[i+1]], dim=1)
