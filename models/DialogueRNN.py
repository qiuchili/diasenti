...
class BiDialogueRNN(nn.Module):
    
    def __init__(self, opt):
        
        super(BiDialogueRNN, self).__init__()
        self.D_m       = sum(opt.input_dims)
        self.D_g       = opt.global_state_dim
        self.D_p       = opt.party_state_dim
        self.D_e       = opt.emotion_rep_dim
        self.D_h       = opt.last_hidden_dim
        self.n_classes = opt.output_dim
        self.device = opt.device
        
        self.dropout   = nn.Dropout(opt.dropout)
        self.dropout_rec = nn.Dropout(opt.dropout+0.15)
        self.dialog_rnn_f = DialogueRNN(self.D_m, self.D_g, self.D_p, self.D_e,opt.listener_state,
                                    context_attention=opt.context_attention_type, D_a = opt.concat_attention_dim, dropout = opt.dropout_rec)
        self.dialog_rnn_r = DialogueRNN(self.D_m, self.D_g, self.D_p, self.D_e,opt.listener_state,
                                    context_attention=opt.context_attention_type, D_a = opt.concat_attention_dim, dropout = opt.dropout_rec)
    ...
    def _reverse_seq(self, X, mask):
        ...
        padded_seq = pad_sequence(xfs)
#        padded = nn.ConstantPad3d((0,0,0,0,0,len(X) - len(padded_seq)),0)(padded_seq)        
        return padded_seq


    def forward(self, in_modalities, att2 = True):
        qmask, umask = in_modalities[-2], in_modalities[-1]
        #qmask: speaker id mask
        #umask: dialogue utterance mask
        ...
        mask_sum = torch.sum(umask, 1).int()
        batch_seq_len = torch.max(mask_sum)
        U = U[:batch_seq_len].to(self.device)
        qmask = qmask[:batch_seq_len].to(self.device)
        umask = umask[:,:batch_seq_len].to(self.device)
        ...
        prob = self.smax_fc(hidden)
        padded_prob = nn.ConstantPad3d((0,0,0,0,0,self.max_seq_len - batch_seq_len),0)(prob)        

        log_prob = F.log_softmax(padded_prob, 2) # seq_len, batch, n_classes
        log_prob = log_prob.transpose(0,1) #  batch, seq_len, n_classes
        return log_prob
...
class BiE2EModel(nn.Module):
    ...
    def _reverse_seq(self, X, mask):
        ...
        for x, c in zip(X_, mask_sum):
            ...
            xfs.append(xf)
        padded_seq = pad_sequence(xfs)
        padded = nn.ConstantPad3d((0,0,0,0,0,seq_len - len(padded_seq)),0)(padded_seq)        
        return padded
