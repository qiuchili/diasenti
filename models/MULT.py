...
#            last_h_v = last_hs = h_vs[-1]
        ...
        output= []
        for _hs in zip(*all_hs):
    
            _h = torch.cat(_hs, dim=1)
    
            # A residual block
            _h_proj = self.proj2(F.dropout(F.relu(self.proj1(_h)), p=self.out_dropout, training=self.training))
            _h_proj += _h
    
            _h_output = self.out_layer(_h_proj)
            output.append(_h_output)
